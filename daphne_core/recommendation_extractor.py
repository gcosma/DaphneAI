...
- FIXED: Range IDs like "8.3-8.4" handled as single recommendations
- FIXED: Parliamentary/Select Committee report formats supported
- ADDED: Paragraph-style recommendations (8.1, 8.2) common in House of Lords reports
- IMPROVED: Better boundary detection for multi-format documents

v3.9 Changes (preserved):
- Page number artifacts in Report 7 recs 2, 11 ("20", "21" embedded in text)
- 5 new regex patterns to strip standalone page numbers at text boundaries
...
- Decimal numbered: "Recommendation 8.1", "8.2", "8.3-8.4"
- Parliamentary reports: Paragraph-style with decimal numbering
...
class StrictRecommendationExtractor:
    ...
    MAX_RECOMMENDATION_LENGTH = 3000  # v4.0: Increased for longer parliamentary recs
    ...
    def __init__(self):
        ...
        self.action_verbs = {
            ...
            'urge', 'call', 'ask', 'request', 'expect', 'believe', 'conclude',
        }
        ...
        self.recommending_entities = [
            ...
            # v4.0: Added parliamentary entities
            r'(?:the\s+)?House\s+of\s+(?:Lords|Commons)',
            r'(?:the\s+)?Select\s+Committee',
            r'(?:the\s+)?Committee',
            r'(?:the\s+)?Ombudsman',
            r'DWP',
            r'WASPI',
    ...
    def clean_text(self, text: str) -> str:
        ...
        # Remove page numbers (but NOT recommendation IDs like 2018/006, R/2023/220, or 8.1)
        # Only remove simple page number fractions like 1/20, 15/84
        ...
        # v4.0 FIX: Be careful not to strip decimal IDs like "8.1"
        text = re.sub(r'\s+\d{1,2}\s*$', '', text)
        text = re.sub(r'^\s*\d{1,2}\s+(?=[A-Z])', '', text)
        text = re.sub(r'\n\s*\d{1,2}\s*\n', '\n', text)
        # v4.0: Only remove page numbers after sentence endings when NOT preceded by a digit
        # This prevents stripping "8.1" -> "8." 
        text = re.sub(r'(?<=[.!?])(?<!\d\.)\s*\d{1,2}\s*(?=[A-Z])', ' ', text)
        text = re.sub(r'(?<=[a-z])\s+\d{1,2}\s+(?=[a-z])', ' ', text)
    ...
    def is_meta_recommendation(self, text: str) -> bool:
        ...
        meta_patterns = [
            ...
            # v4.0: Additional meta patterns for parliamentary reports
            r'^the\s+committee\s+(?:has\s+)?(?:made|published)\s+\d+\s+recommendations',
            r'^this\s+chapter\s+(?:sets\s+out|contains)',
            r'^we\s+set\s+out\s+(?:our|the)\s+recommendations',
        ]
    ...
    def _extract_verb(self, text: str) -> str:
        ...
        # v4.0: Check for "urge", "call on", "ask" patterns common in parliamentary reports
        urge_match = re.search(r'\b(?:urge|call\s+on|ask)\s+(?:the\s+)?(?:government|minister)?\s*(?:to\s+)?(\w+)', text_lower)
        if urge_match:
            verb = urge_match.group(1)
            if verb in self.action_verbs:
                return verb
    ...
    def _is_section_boundary(self, text: str, position: int) -> bool:
        ...
        # Two-level numbering followed by text (but NOT if it's a recommendation ID)
        two_level_match = re.match(r'^(\d+\.\d+)\s+[A-Z]', remaining_stripped)
        if two_level_match:
            # Check if this looks like a recommendation header
            potential_rec = remaining_stripped[:100].lower()
            if not ('recommend' in potential_rec or 'government' in potential_rec):
                return True
        
        # HSSIB safety response separator
        if remaining_stripped.startswith('HSSIB proposes'):
            return True
        
        # Common finding starters
    ...
    def _find_recommendation_end_generic(self, text: str, start_pos: int, next_rec_pos: int) -> int:
        ...
        # v4.0: Check for inline recommendation patterns (including decimal)
        inline_rec_pattern = re.compile(
            r'\bRecommendation\s+\d+(?:\.\d+)?(?:-\d+(?:\.\d+)?)?[:\s]+(?:All|The|NHS|Every|Provider|Trust|Board|ICS|CQC|DHSC|Ward|More|Professional|Except|TEWV|Middlesbrough|We|Ministers?|Government)',
            re.IGNORECASE
        )
        ...
        # v4.0: Check for "Government response to recommendation" pattern (indicates we've hit response section)
        gov_response_pattern = re.compile(
            r'\bGovernment\s+response\s+to\s+recommendation',
            re.IGNORECASE
        )
        gov_match = gov_response_pattern.search(text[search_start:search_limit])
        if gov_match:
            potential_end = search_start + gov_match.start()
            if potential_end < search_limit:
                logger.debug(f"Found government response boundary at position {potential_end}")
                search_limit = potential_end
    ...
    def is_valid_recommendation(self, text: str, is_numbered_rec: bool = False) -> Tuple[bool, float, str, str]:
        ...
        # v4.0: Decimal recommendation format with colon: "Recommendation 8.1:" or "8.3-8.4:"
        decimal_colon_match = re.match(
            r'^(?:Recommendation\s+)?(\d+\.\d+(?:-\d+(?:\.\d+)?)?)[:\s]+(.+)',
            cleaned,
            re.IGNORECASE | re.DOTALL,
        )
        if decimal_colon_match:
            rec_num = decimal_colon_match.group(1)
            rec_text = decimal_colon_match.group(2)
            verb = self._extract_verb(rec_text)
            return True, 0.98, f'decimal_recommendation_{rec_num}', verb
        
        # v4.0: Decimal recommendation format without colon: "Recommendation 8.1 The idea..."
        ...
        if decimal_match:
            rec_num = decimal_match.group(1)
            rec_text = decimal_match.group(2)
            verb = self._extract_verb(rec_text)
            return True, 0.98, f'decimal_recommendation_{rec_num}', verb
        
        # Standard numbered recommendation with colon: "Recommendation 1:"
        ...
        # v4.0: "We urge" / "We call on" patterns for parliamentary reports
        if re.search(r'\bwe\s+(?:urge|call\s+on|ask)\b', text_lower):
            verb = self._extract_verb(cleaned)
            return True, 0.90, 'we_urge', verb
    ...
    def extract_recommendations(self, text: str, min_confidence: float = 0.75) -> List[Dict]:
        ...
        # =======================================================================
        # PHASE 3: v4.0 - Try decimal format (8.1, 8.2, 8.3-8.4)
        # =======================================================================
        # Pattern 1: "Recommendation 8.1" with explicit "Recommendation" keyword
        decimal_explicit_pattern = re.compile(
            r'Recommendation\s+(\d+\.\d+(?:-\d+(?:\.\d+)?)?)\s+',
            re.IGNORECASE
        )
        decimal_explicit_matches = list(decimal_explicit_pattern.finditer(cleaned_full_text))
        
        if decimal_explicit_matches:
            logger.info(f"Found {len(decimal_explicit_matches)} explicit decimal recommendation headings")
            recommendations = self._extract_from_matches(
                cleaned_full_text, decimal_explicit_matches, 'decimal', min_confidence
            )
            if recommendations:
                return self._deduplicate(recommendations, by_rec_number=True)
        
        # Pattern 2: Standalone decimal at start of line (less common)
        decimal_standalone_pattern = re.compile(
            r'(?:^|\n)\s*(\d+\.\d+(?:-\d+(?:\.\d+)?)?)[:\s]+(?=[A-Z])',
            re.MULTILINE
        )
        decimal_standalone_matches = list(decimal_standalone_pattern.finditer(cleaned_full_text))
        
        # Filter to only include matches that look like recommendation IDs
        valid_decimal_matches = []
        for match in decimal_standalone_matches:
            rec_id = match.group(1)
            start = match.start()
            # Check context - should have recommendation-like content after
            following_text = cleaned_full_text[match.end():match.end()+100].lower()
            if any(word in following_text for word in ['should', 'must', 'government', 'recommend', 'minister']):
                valid_decimal_matches.append(match)
        
        if valid_decimal_matches:
            logger.info(f"Found {len(valid_decimal_matches)} standalone decimal recommendation headings")
            recommendations = self._extract_from_matches(
                cleaned_full_text, valid_decimal_matches, 'decimal', min_confidence
            )
            if recommendations:
                return self._deduplicate(recommendations, by_rec_number=True)
        
        # =======================================================================
        # PHASE 4: Standard "Recommendation N" format (including with colon)
        # =======================================================================
        ...
        # =======================================================================
        # PHASE 5: Fallback to sentence extraction
        # =======================================================================
        ...
        for idx, match in enumerate(matches):
            ...
            if is_rec and confidence >= min_confidence:
                ...
                if pattern_type == 'hsib_2023':
                    ...
                    rec_num = match.group(1)  # e.g., "8.1", "8.3-8.4"
    ...
    def _deduplicate(self, recommendations: List[Dict], by_rec_number: bool = False) -> List[Dict]:
        ...
        if by_rec_number:
            ...
            def sort_key(r):
                ...
                if '/' in str(num):
                    # HSIB format: "2018/006" or "R/2023/220"
                    ...
                    # v4.0: Decimal format: "8.1", "8.3-8.4"
                    # Extract first number for sorting
                    base_num = str(num).split('-')[0]  # "8.3-8.4" -> "8.3"
                    ...
                    # Range format without decimal: "3-4"
                    parts = str(num).split('-')
                    try:
